...
//! shares (`agUSDC`) to underlying USDC, and accumulates the protocol's continuous yield skim in
...
impl YieldStrategy {
    ...
    /// equivalent of all user-held agUSDC.
    ...
    /// Translate `usdc` -> agUSDC shares using the current index. Rounds down (favours protocol).
    ...
    /// Translate agUSDC shares -> USDC. Rounds down (favours protocol on withdrawal fee math).
